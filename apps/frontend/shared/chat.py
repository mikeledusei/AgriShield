...
def render_chat(county_name: str = None, placeholder_text: str = "Ask Gria about agricultural risks..."):
    ...
    with st.form("gria_chat_form", clear_on_submit=True):
...
def gria_quick_prompts(county_name: str = None):
    """Display quick prompt buttons."""
    prompts = [
        "What is the current risk for this county?",
        "What are the main risk drivers?",
        "What should farmers do to prepare?",
        "How does this compare to last year?",
    ]
    if county_name:
        prompts.insert(0, f"Give me a detailed risk report for {county_name}")

    cols = st.columns(len(prompts))
    for i, prompt in enumerate(prompts):
        if cols[i].button(prompt, key=f"prompt_{i}", use_container_width=True):
            st.session_state.gria_prompt = prompt
            st.rerun()
